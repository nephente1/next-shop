...
export async function getMovie(id: string) {
  const moviesWithCategory = await moviesList.map((movie) => ({
    ...movie,
    category: 'movies', price: 100,
  }));
  
  const movie = await moviesWithCategory.find((movie) => movie.id === id);
  if (!movie) {
...
export const moviesList = [
  {
    "title": "Emilia Perez",
    "description": "Cartel leader Emilia enlists Rita, an unappreciated lawyer, to help fake her death so that she can finally live authentically as her true self.",
    "year": "2024",
    "image": "emilia_perez.jpg",
    "id": "0250111"
  },
  {
