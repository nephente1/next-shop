...
export default async function Home() {
  ...
  const renderCategoriesBoxes = ([...categories, "movies"].map((category: string, i: number) => {
    ...
    return (
      <a key={i} href={`/categories/${category}`} style={{ textDecoration: 'none' }} className="border border-sky-400 p-5 m-2 radius flex flex-wrap flex-col justify-between items-center shadow-lg basis-60 flex-auto cursor-pointer hover:border-pink-600">
        <div title={category} className="text-sky-600 text-xl capitalize text-center text-ellipsis overflow-hidden w-60 line-clamp-2 font-bold">{category}</div>
